# Plan Run Manifest

## Task

- Request Summary: <what needs to be planned>
- Origin Requirements Path: <docs/brainstorms/... | "none">
- Design Path: <docs/designs/... | docs/architecture/... | "none">
- Existing Plan Path: <docs/plans/... | "none">
- Plan Mode: <create | deepen | infer>

## Planning Scope

- Depth: <lightweight | standard | deep | infer>
- Must Resolve During Planning:
  - <question or "infer">
- Allowed To Defer To Execution:
  - <question or "infer">

## Research Expectations

- Run Local Research: <yes | no | infer>
- Run External Research: <yes | no | conditional>
- External Risk Areas:
  - <security | payments | migrations | external APIs | "none">

## Output Expectations

- Target Plan Path: <docs/plans/...>
- Output Mode: <session_local | write_artifact>
- Run Critique Stack: <yes | no | conditional>
- Expected Transition:
  - <proceed -> cmon:work | revise -> cmon:plan | block | infer>

## Known Constraints

- <constraint or "none">
