...
2026-04-09 follow-up: approval gates must be real human gates.
Dogfooding showed that agents can otherwise create `status: approved` artifacts themselves and continue without actual human approval.
...
- creating an `approved` design approval artifact without an explicit user approval or waiver quote
...
- creating an `approved` package approval artifact without an explicit user approval or waiver quote

## 5. Human Approval Gates

Approval gates are hard stops.
Agents may prepare approval request artifacts, but the request status must be `pending_user_approval` until the user explicitly approves, rejects, requests changes, or waives the gate.

Valid `approved` or `waived_by_user` artifacts require:

- status of `approved` or `waived_by_user`
- human approver identity
- approval source
- exact user approval or waiver quote
- recording agent identity

Invalid approval evidence:

- challenge passed
- agent says the artifact is acceptable
- user approved the original task before seeing the challenged artifact
- user gave broad momentum language before design or package artifacts existed
- approval artifact was generated by the agent with no user quote
